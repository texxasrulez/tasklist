...
class tasklist extends rcube_plugin
{
        ...
        switch ($action) {
            ...
            case 'edit':
                ...
                if ($success = $this->driver->edit_task($rec)) {
                    ...
                    $new_task['tempid'] = (isset($rec['id']) ? $rec['id'] : '');
                    $refresh[] = $new_task;
                    $this->cleanup_task($rec);
                    ...
                    if ($clone && $this->driver->create_task($clone)) {
                        ...
                        $refresh[] = $new_clone;
                        $this->driver->clear_alarms((isset($rec['id']) ? $rec['id'] : ''));
                    }

                    // move all childs if list assignment was changed
                    if (!empty($rec['_fromlist']) && !empty($rec['list']) && $rec['_fromlist'] != $rec['list']) {
                        foreach ($this->driver->get_childs(['id' => (isset($rec['id']) ? $rec['id'] : ''), 'list' => $rec['_fromlist']], true) as $cid) {
            ...
            case 'move':
                foreach ((array)(isset($rec['id']) ? $rec['id'] : '') as $id) {
                    $r = $rec;
                    $r['id'] = $id;
                        ...
                        if ($mode == 1) {  // delete all childs
                            if ($this->driver->delete_task($child, false)) {
                                if ($this->driver->undelete) {
                                    $_SESSION['tasklist_undelete'][(isset($rec['id']) ? $rec['id'] : '')][] = $cid;
                                }
                            } else {
                                $success = false;
                            }
            ...
            case 'undelete':
                if ($success = $this->driver->undelete_task($rec)) {
                    $refresh[] = $this->driver->get_task($rec);
                    foreach ((array)$_SESSION['tasklist_undelete'][(isset($rec['id']) ? $rec['id'] : '')] as $cid) {
            ...
            case 'collapse':
                foreach (explode(',', (isset($rec['id']) ? $rec['id'] : '')) as $rec_id) {
    ...
    private function prepare_task($rec)
    {
        ...
        if (!isset($rec['description']) || $rec['description'] === 'null') {
            $rec['description'] = '';
        }
        ...
        $rec['attachments'] = $handler->attachments_set(self::SESSION_KEY, (isset($rec['id']) ? $rec['id'] : ''), $rec['attachments'] ?? []);
        ...
        if ((isset($rec['id']) && is_numeric($rec['id'])) && (isset($rec['id']) ? $rec['id'] : '') < 0) {
            unset((isset($rec['id']) ? $rec['id'] : ''));
        }
    ...
    private function handle_recurrence(&$rec, $old)
    {
        ...
        if ($this->driver->is_complete($rec) && $old && !$this->driver->is_complete($old) && !empty($rec['recurrence'])) {
            ...
            if (!empty($updates)) {
                ...
                $clone['parent_id'] = (isset($rec['id']) ? $rec['id'] : '');
    ...
    private function tasks_data($records)
    {
        ...
        foreach ($records as $rec) {
            if (!empty($rec['parent_id'])) {
                $this->task_tree[(isset($rec['id']) ? $rec['id'] : '')] = $rec['parent_id'];
            }
    ...
    private function encode_task(&$rec)
    {
        ...
        if (in_array((isset($rec['id']) ? $rec['id'] : ''), $this->collapsed_tasks)) {
            $rec['collapsed'] = true;
        }
        ...
        $this->task_titles[(isset($rec['id']) ? $rec['id'] : '')] = $rec['title'] ?? '';
    }
    ...
    private function task_walk_tree(&$rec)
    {
        $rec['_depth'] = 0;
        $parent_titles = [];
        $parent_id = $this->task_tree[(isset($rec['id']) ? $rec['id'] : '')] ?? null;
        while ($parent_id) {
            $rec['_depth']++;
