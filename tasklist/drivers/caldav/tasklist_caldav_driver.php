...
class tasklist_caldav_driver extends tasklist_driver
{
    ...
    public function get_childs($prop, $recursive = false)
    {
        if (is_string($prop)) {
            ...
            $prop = ['uid' => $this->ensure_uid($task), 'list' => $task['list']];
        } else {
            $this->_parse_id($prop);
        }
    ...
    */
    ...
    /**
     * Ensure a UID string exists for a given task or id
     */
    private function ensure_uid($task)
    {
        if (is_array($task)) {
            if (!empty($this->ensure_uid($task))) {
                return $this->ensure_uid($task);
            }
            if (!empty($task['id']) && is_string($task['id'])) {
                $id = $task['id'];
                $pos = strrpos($id, ':');
        ...
        if (is_string($task)) {
            $pos = strrpos($task, ':');
            return $pos !== false ? substr($task, $pos + 1) : $task;
        }
        return null;
    }
...
//            'location' => $record['location'],
    ...
    public function edit_task($task)
    {
        ...
        if (!empty($task['_fromlist']) && ($fromfolder = $this->get_folder($task['_fromlist']))) {
            if (!$fromfolder->move($this->ensure_uid($task), $folder)) {
                return false;
            }
        ...
        if (!empty($task['id'])) {
            $old = $folder->get_object($this->ensure_uid($task));
            if (!$old) {
                return false;
            }
        ...
        $saved = $folder->save($object, 'task', !empty($old) ? $this->ensure_uid($task) : null);
        ...
        $this->tasks[$this->ensure_uid($task)] = $task;
    ...
    public function move_task($task)
    {
        ...
        if (!empty($task['_fromlist']) && ($fromfolder = $this->get_folder($task['_fromlist']))) {
            return $fromfolder->move($this->ensure_uid($task), $folder);
        }
    ...
    public function delete_task($task, $force = true)
    {
        ...
        $status = $folder->delete($this->ensure_uid($task), $force);
