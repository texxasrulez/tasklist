...
class tasklist_rounddav_storage_dav extends kolab_storage_dav
{
    /**
    ...
    public function folder_update(&$prop)
    {
        ...
        $use_slug = $rcube->config->get('rounddav_tasklist_slug_uri', true);

        if (!empty($prop['uri'])) {
            $uid = trim((string) $prop['uri']);
        } elseif ($use_slug && !empty($prop['name'])) {
            $uid = $this->slug_from_name($prop['name']);
        }
    ...
    /**
     * Build a URL-friendly tasklist URI from a display name.
     */
    protected function slug_from_name($name)
    {
...
class tasklist_rounddav_driver extends tasklist_driver
{
    ...
    public function __construct($plugin)
    {
        ...
        $url = (string) $this->rc->config->get('tasklist_caldav_server', '');
        if ($url === '') {
        ...
        if ($url === '') {
            rcube::raise_error(
                [
                    ...
                    'file' => __FILE__,
                    'line' => __LINE__,
                    'message' => "RoundDAV tasklist driver misconfiguration: set 'tasklist_caldav_server' or 'rounddav_base_url'.",
                ],
                true,
                false
            );
            throw new RuntimeException("RoundDAV tasklist driver misconfiguration: missing 'tasklist_caldav_server'/'rounddav_base_url'.");
        }

        $url = rtrim($url, '/') . '/';

        $this->storage = new tasklist_rounddav_storage_dav($url);
    ...
    public function get_lists($filter = 0, &$tree = null)
    {
        ...
        $favorites = $this->rc->config->get('rounddav_tasklist_favorites', []);
        $favorites = $this->resolve_favorite_ids($favorites, $lists);
        if (!empty($favorites)) {
            $ordered = [];
            foreach ($favorites as $fav_id) {
                if (isset($lists[$fav_id])) {
                    $ordered[$fav_id] = $lists[$fav_id];
                }
            }

            foreach ($lists as $id => $props) {
                if (!isset($ordered[$id])) {
                    $ordered[$id] = $props;
                }
            }
    ...
    /**
     * Resolve favorite identifiers to list IDs.
     *
     * @param array $favorites List of IDs, names, or URIs
     * @param array $lists     List properties indexed by ID
     *
     * @return array Ordered list of list IDs
     */
    protected function resolve_favorite_ids($favorites, $lists)
    {
        if (empty($favorites) || !is_array($favorites) || empty($lists)) {
            return [];
        }
        ...
        foreach ($this->folders as $id => $folder) {
            if (!isset($lists[$id]) || empty($folder->href)) {
                continue;
            }

            $uri = trim((string) $folder->href, '/');
