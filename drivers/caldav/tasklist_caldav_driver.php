...
class tasklist_caldav_driver extends tasklist_driver
{
    ...
    public function count_tasks($lists = null)
    {
        ...
        if ($lists === true) {
            $lists = array_keys($this->_read_lists());
        }
        if (!is_array($lists) && !($lists instanceof Traversable)) {
            $lists = (array)$lists;
        }
...
//            'location' => $record['location'],
    ...
    private function _from_rcube_task($task, $old = [])
    {
        ...
        $task_complete = isset($task['complete']) ? (float)$task['complete'] : 0.0;
$object['complete'] = (int)round($task_complete * 100);
if ($task_complete >= 1.0) {
