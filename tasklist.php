...
class tasklist extends rcube_plugin
{
    ...
    public function task_action()
    {
        ...
        // Fallback for non-XHR environments: navigate back to tasks screen so the UI handles the update
        $is_xhr = !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest';
        if (!$is_xhr) {
            $this->rc->output->redirect(['_task' => 'tasks']);
            return;
        }
        ...
        // Fallback: if this wasn't an XHR (some setups open the JSON in the window), redirect back to Tasks UI
        $is_xhr = !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest';
        if (!$is_xhr) {
            $this->rc->output->redirect(['_task' => 'tasks']);
            return;
    ...
    private function prepare_task($rec)
    {
        ...
        $rec['attachments'] = $handler->attachments_set(self::SESSION_KEY, ($rec['id'] ?? ($rec['uid'] ?? '')), $rec['attachments'] ?? []);
